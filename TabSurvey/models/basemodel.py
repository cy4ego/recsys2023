...
from utils.io_utils import save_predictions_to_file, save_model_to_file, save_infers_to_file
...
class BaseModel:
    ...
    def infer(self, X: np.ndarray) -> tp.Tuple[np.ndarray, np.ndarray]:
        """
        Returns the regression value or the concrete classes of binary / multi-class-classification tasks.
        (Save predictions to self.predictions)
        ...
        if self.args.objective == "regression":
            self.infers = self.model.predict(X)
        elif self.args.objective == "classification" or self.args.objective == "binary":
            prediction_probabilities = self.prediction_probabilities
            self.infers = self.predict_proba(X)
    ...
    def save_model_and_predictions(self, y_true: np.ndarray, filename_extension=""):
        ...
        if self.infers is not None:
            self.save_infers(filename_extension)
    ...
    def save_infers(self, filename_extension=''):
        save_infers_to_file(self.infers, self.args, filename_extension)
