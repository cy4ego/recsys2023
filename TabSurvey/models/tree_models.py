...
class XGBoost(BaseModel):
    ...
    def __init__(self, params, args):
        ...
        self._num_boost_round = self.args.epochs if self.args.epochs < 5 else self.args.epochs * 4000

    def fit(self, X, y, X_val=None, y_val=None):
        train = xgb.DMatrix(X, label=y)
        val = xgb.DMatrix(X_val, label=y_val)
        eval_list = [(val, "eval")]
        self.model = xgb.train(self.params, train, num_boost_round=self._num_boost_round, evals=eval_list,
                               early_stopping_rounds=self.args.early_stopping_rounds * 20,
...
class CatBoost(BaseModel):
    ...
    def __init__(self, params, args):
        ...
        self.params["iterations"] = self.args.epochs if self.args.epochs < 5 else self.args.epochs * 4000
        self.params["eval_metric"] = "Logloss"
        self.params["od_type"] = "Iter"
        self.params["od_wait"] = self.args.early_stopping_rounds * 20
        self.params["verbose"] = self.args.logging_period
        self.params["train_dir"] = self.args.output_dir + "CatBoost/" + self.args.dataset + "/catboost_info"
        if not os.path.exists(self.params['train_dir']):
            os.makedirs(self.params['train_dir'])
...
class LightGBM(BaseModel):
    ...
    def __init__(self, params, args):
        ...
        self.params["verbosity"] = -1
        self.params['n_jobs'] = 40
        self.params['feature_fraction'] = 0.4
        ...
        # self.params['feature_fraction'] = 0.6
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        self.model = lgb.train(self.params, train, num_boost_round=self._num_boost_round, valid_sets=[val],
                               valid_names=["eval"], callbacks=[lgb.early_stopping(self.args.early_stopping_rounds*20),
