...
# cy4ego, 2023.05.05, Added for RecSys2023 Challenge
def save_infers_to_file(arr, args, extension=""):
    filename = get_output_path(args, directory="infers", filename="r", extension=extension, file_type="npy")
    np.save(filename, arr)
...
def get_output_path(args, filename, file_type, directory=None, extension=None):
    ...
    dir_path = args.output_dir + args.model_name + "/" + args.dataset
...
def get_predictions_from_file(args):
    dir_path = args.output_dir + args.model_name + "/" + args.dataset + "/predictions"
