...
def cross_validation(model, X, y, te_X, args, save_model=False):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X, y)):
        ...
        if te_X is not None and len(te_X) > 0:
            curr_model.infer(te_X)
        test_timer.end()
...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        sc, time = cross_validation(model, self.X, self.y, None, self.args)
...
def main(args):
    ...
    cross_validation(model, X, y, te_X, args, save_model=True)
...
def main_once(args):
    ...
    sc, time = cross_validation(model, X, y, te_X, args, save_model=True)
    print(sc.get_results())
    print(time)
